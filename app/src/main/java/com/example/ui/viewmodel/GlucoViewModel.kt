...
class GlucoViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val insulinRecords: StateFlow<List<InsulinRecord>> = activeProfileId
        .flatMapLatest { pid -> repository.allInsulinRecords(pid) }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    @OptIn(kotlinx.coroutines.ExperimentalCoroutinesApi::class)
    val glucoseReadings: StateFlow<List<GlucoseReading>> = activeProfileId
        .flatMapLatest { pid -> repository.allGlucoseReadings(pid) }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    @OptIn(kotlinx.coroutines.ExperimentalCoroutinesApi::class)
    val reminders: StateFlow<List<Reminder>> = activeProfileId
        .flatMapLatest { pid -> repository.allReminders(pid) }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    @OptIn(kotlinx.coroutines.ExperimentalCoroutinesApi::class)
    val refillLogs: StateFlow<List<CartridgeRefillLog>> = activeProfileId
        .flatMapLatest { pid -> repository.allRefillLogs(pid) }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    @OptIn(kotlinx.coroutines.ExperimentalCoroutinesApi::class)
    val bloodPressureRecords: StateFlow<List<BloodPressureRecord>> = activeProfileId
        .flatMapLatest { pid -> repository.allBloodPressureRecords(pid) }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    @OptIn(kotlinx.coroutines.ExperimentalCoroutinesApi::class)
    val stepRecords: StateFlow<List<StepCountRecord>> = activeProfileId
        .flatMapLatest { pid -> repository.allStepRecords(pid) }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    fun loginAsGuest() {
        ...
        _isAppLocked.value = false
        switchActiveProfile("Guest Patient")
    }
    ...
    fun login(usernameOrEmail: String, password: String, rememberMeChecked: Boolean = false): Boolean {
        _loginError.value = null
        ...
        if (isEmail) {
            targetEmail = trimmed.lowercase()
            val mappedUser = prefs.getString("email_to_user_$targetEmail", null)
            resolvedUsername = if (!mappedUser.isNullOrEmpty()) mappedUser else targetEmail.substringBefore("@")
        } else {
            resolvedUsername = trimmed
            val mappedEmail = prefs.getString("user_email_$resolvedUsername", null)
            targetEmail = if (!mappedEmail.isNullOrEmpty()) {
                mappedEmail
            } else {
                "${resolvedUsername.lowercase().replace(" ", "_")}@glucolog.app"
            }
        }
        ...
        try {
            ...
            auth.signInWithEmailAndPassword(targetEmail, safePassword)
                .addOnSuccessListener { result ->
                    val finalEmail = result.user?.email ?: targetEmail
                    val finalUser = if (isEmail && prefs.getString("email_to_user_$finalEmail", null) != null) {
                        prefs.getString("email_to_user_$finalEmail", resolvedUsername) ?: resolvedUsername
                    } else {
                        resolvedUsername
                    }

                    prefs.edit()
                        .putString("user_pass_$finalUser", password)
                        .putString("user_email_$finalUser", finalEmail)
                        .putString("email_to_user_${finalEmail.lowercase()}", finalUser)
                        .apply()

                    _isAdmin.value = false
                    _isGuestMode.value = false
                    _loggedInUser.value = finalUser
                    _isLoggedIn.value = true
                    _loginError.value = null
                    
                    switchActiveProfile(finalUser)
                    ...
                    fetchGoogleDriveTokenAutomatically(getApplication(), finalEmail)
                ...
                .addOnFailureListener { e ->
                    ...
                    if (savedPassword != null && savedPassword == password) {
                        ...
                        switchActiveProfile(resolvedUsername)
                        
                        saveRememberMeConfig(prefs, rememberMeChecked, trimmed, password)
                        clearOnlyAutoGeneratedDummyData()
                        android.widget.Toast.makeText(getApplication(), "Successfully authorized (cached offline)!", android.widget.Toast.LENGTH_SHORT).show()
                    } else {
                        val errorMsg = when {
                            e is com.google.firebase.FirebaseNetworkException -> "Network error. Please check your internet connection."
                            e is com.google.firebase.auth.FirebaseAuthInvalidCredentialsException -> "Invalid email or password. Please verify your credentials."
                            e is com.google.firebase.auth.FirebaseAuthInvalidUserException -> {
                                if (!isEmail) "No local session found for '$trimmed'. Please log in with your email address."
                                else "No account exists for this email. Please register first."
                            }
                            else -> e.localizedMessage ?: "Authentication failed. Please check your credentials."
                        }
            ...
            if (savedPassword == null) {
                _loginError.value = "Account does not exist locally. Please connect to internet or register."
    ...
    fun registerUser(username: String, email: String, password: String): Boolean {
        ...
        if (prefs.contains("user_pass_$trimmedUser") && prefs.getString("user_pass_$trimmedUser", "") != password) {
            _loginError.value = "Username already exists locally. Please choose another"
            return false
        }
            ...
            auth.createUserWithEmailAndPassword(trimmedEmail, safePassword)
                ...
                .addOnFailureListener { e ->
                    if (e is com.google.firebase.auth.FirebaseAuthUserCollisionException) {
                        android.util.Log.d("FirebaseAuth", "Firebase account already exists. Credentials linked locally.")
                    } else {
    ...
    fun switchActiveProfile(userName: String) {
        val displayName = if (userName.isEmpty() || userName == "Guest Patient") {
            "Guest Patient"
        } else {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            var profile = repository.getProfileByUsername(displayName)
            if (profile == null) {
                profile = UserProfile(userName = displayName, isActive = true)
                repository.insertOrUpdateProfile(profile)
                profile = repository.getProfileByUsername(displayName)
            } else {
                repository.insertOrUpdateProfile(profile.copy(isActive = true))
            }
            val pid = profile?.id ?: 1
            withContext(Dispatchers.Main) {
    ...
    private fun ensureMinimumSetup() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val anyRem = repository.getAnyReminderSync(pid)
            if (anyRem == null) {
                val defaultRems = listOf(
                    Reminder(profileId = pid, reminderType = "Insulin", label = "Morning Rapid Dose", hour = 8, minute = 0),
                    Reminder(profileId = pid, reminderType = "Insulin", label = "Lunch Time Dose", hour = 12, minute = 30),
                    Reminder(profileId = pid, reminderType = "Insulin", label = "Evening Long-acting Dose", hour = 21, minute = 0),
                    Reminder(profileId = pid, reminderType = "Blood Sugar Check", label = "Fasting Glucose Check", hour = 7, minute = 0),
                    Reminder(profileId = pid, reminderType = "Blood Sugar Check", label = "Bedtime Glucose Check", hour = 22, minute = 0)
                )
    ...
    fun saveRefillLog(log: CartridgeRefillLog) {
        viewModelScope.launch {
            repository.insertRefillLog(log.copy(profileId = activeProfileId.value))
        }
    }
    ...
    fun clearAllRefillLogs() {
        viewModelScope.launch {
            repository.clearAllRefillLogs(activeProfileId.value)
            recalculateCartridgeRemaining()
        }
    }
    ...
    private suspend fun recalculateCartridgeRemaining() {
        ...
        val refills = repository.allRefillLogs(activeProfileId.value).first()
        val doses = repository.allInsulinRecords(activeProfileId.value).first()
    ...
    fun clearAllLogs(onComplete: () -> Unit) {
        val pid = activeProfileId.value
        viewModelScope.launch {
            repository.clearAllGlucoseReadings(pid)
            repository.clearAllInsulinRecords(pid)
            repository.clearAllBloodPressureRecords(pid)
            repository.clearAllRefillLogs(pid)
            recalculateCartridgeRemaining()
            onComplete()
        }
    }
    ...
    fun clearAllAppData(onComplete: () -> Unit) {
        ...
        viewModelScope.launch {
            ...
            repository.clearAllBloodPressureRecords(pid)
            repository.clearAllRefillLogs(pid)
            repository.clearAllReminders(pid)
            
            // Reset the current active profile settings to defaults
            val currentProf = repository.getProfileSync()
            if (currentProf != null) {
                repository.insertOrUpdateProfile(
                    UserProfile(
    ...
    fun exportAppDataToJSON(onComplete: (String?) -> Unit) {
        ...
        viewModelScope.launch {
            try {
                ...
                // 1. Glucose readings
                val glucoseList = repository.allGlucoseReadings(pid).first()
                ...
                // 2. Insulin records
                val insulinList = repository.allInsulinRecords(pid).first()
                ...
                val bpList = repository.allBloodPressureRecords(pid).first()
                val bpArr = JSONArray()
                for (item in bpList) {
                    val obj = JSONObject()
                ...
                // 4. Refill logs
                val refillList = repository.allRefillLogs(pid).first()
                ...
                // 5. Reminders
                val reminderList = repository.allReminders(pid).first()
                ...
                val stepsList = repository.allStepRecords(pid).first()
    ...
    fun importAppDataFromJSON(jsonString: String, onComplete: (Boolean) -> Unit) {
        ...
        viewModelScope.launch {
            try {
                ...
                // Clear current database records only for the active profile
                repository.clearAllGlucoseReadings(pid)
                repository.clearAllInsulinRecords(pid)
                repository.clearAllBloodPressureRecords(pid)
                repository.clearAllRefillLogs(pid)
                repository.clearAllReminders(pid)
                repository.clearAllStepRecords(pid)

                // 1. Profiles (Update active profile settings)
                val profilesArr = root.optJSONArray("profiles")
                if (profilesArr != null && profilesArr.length() > 0) {
                    val obj = profilesArr.getJSONObject(0)
                    val currentProf = repository.getProfileSync()
                    if (currentProf != null) {
                        repository.insertOrUpdateProfile(
                            com.example.data.model.UserProfile(
                                id = currentProf.id,
                                userName = currentProf.userName,
                                doctorName = obj.optString("doctorName", currentProf.doctorName),
                                doctorEmail = obj.optString("doctorEmail", currentProf.doctorEmail),
                                doctorPhone = obj.optString("doctorPhone", currentProf.doctorPhone),
                                targetGlucoseMin = obj.optDouble("targetGlucoseMin", currentProf.targetGlucoseMin),
                                targetGlucoseMax = obj.optDouble("targetGlucoseMax", currentProf.targetGlucoseMax),
                                glucoseUnit = obj.optString("glucoseUnit", currentProf.glucoseUnit),
                                isActive = true,
                                cartridgeCapacity = obj.optDouble("cartridgeCapacity", currentProf.cartridgeCapacity),
